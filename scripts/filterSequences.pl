...
my $work_dir = shift;
my $prefix = shift;
my $ampliconSize = shift;
my $noOfClusters = shift;
...
my $reads_len = 0;
my $counts_len = 0;
open (INFO, ">$work_dir/$sample.$prefix.clusters.details.txt") or die "Cannot write $work_dir/$sample.$prefix.clusters.details.txt\n";
...
if ( -e $clusters_file ){
	...
	# my $seqio = Bio::SeqIO->new(-file => "$clusters_file", -format => "fasta");
	# LOOP8: while(my $seq = $seqio -> next_seq) {
	# 	$id = $seq->id;
	# 	@words = split(/:/, $id);
	# 	$variants{$seq->seq()} = $words[1];
	# 	$variants_ids{$seq->seq()} = $id;
	# 	$variants_per{$seq->seq()} = $words[2];
	# 	$lengths{$seq->seq()} = length($seq->seq());
	# }
	...
	foreach my $seq (sort {$variants{$b} <=> $variants{$a}} keys %variants){
		...
		$count_to_finish++;
		if ($count_to_finish <= $noOfClusters){
			my @nucl = split //, $seq;
			...
			print INFO "$id\t$variants{$seq}\t$variants_per{$seq}\t$lengths{$seq}\t$details{$seq}\t$seq\n";
		}
		else{
			$count_low++;
...
open (STATS, ">$work_dir/$sample.$prefix.clusters.stats.txt") or die "Cannot write $work_dir/$sample.$prefix.clusters.stats.txt\n";
