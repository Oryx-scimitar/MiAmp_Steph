...
This snakemake pipeline and the interative perl scripts have been developed to analyse the MHCI amplicon sequencing data or any targeted amplicon sequencing data (such as 16S/18S) using custom primers or any similar custom amplicon sequencing data.
...
We need the following packages installed in the environment: Perl, Blast, Flash, Sickle.
There are two ways to setup the conda environment. 1) using yml file 2) manually installing all packages.
1. Using yml file: I have attached yml file. Following command will help to install from this file: `conda env create -f conda_env_amplicon.yml`
2. Manually creating env and installing packages using following commands. “amplicon” is the name of the environment, you can choose any name.
```
...
`source activate amplicon` or `conda activate amplicon`
...
- **fasta**:
    ...
    - The available database for the primers used in this study is the SILVA database. You can use your own database.
    - To index database, use this command: `makeblastdb -in <database.fa> -dbtype nucl` or `makeblastdb -in <database.fasta> -dbtype nucl`
